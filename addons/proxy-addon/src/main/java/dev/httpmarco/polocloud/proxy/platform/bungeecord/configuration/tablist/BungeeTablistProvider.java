...
public abstract class BungeeTablistProvider implements TablistFormatter {
    ...
    public BungeeTablistProvider(BungeeCordPlatformPlugin platform) {
    ...
    public String formatPlaceholders(String template, CloudPlayer cloudPlayer) {
        return template
                ...
                .replace("%onlinePlayers%", String.valueOf(this.platform.getProxy().getOnlineCount()))
                .replace("%maxOnlinePlayers%", String.valueOf(this.platform.getProxy().getConfig().getPlayerLimit()));
    }
}