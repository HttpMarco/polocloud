...
public class CloudServiceStartListener {
    ...
    public CloudServiceStartListener(ProxyServer server, MessagesConfiguration messagesConfiguration, EventNode node) {
        node.addListener(CloudServiceStartEvent.class, event -> {

            server.getAllPlayers().stream().filter(player -> player.hasPermission("polocloud.addon.notify")).forEach(player ->
                    player.sendMessage(MiniMessage.miniMessage().deserialize(messagesConfiguration.serviceStarting().replace("%service%", event.cloudService().name()))));
        });
    }
}
