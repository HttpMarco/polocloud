...
public class CloudServiceShutdownListener {
    ...
    public CloudServiceShutdownListener(MessagesConfiguration messagesConfiguration, EventNode node) {
        node.addListener(CloudServiceShutdownEvent.class, event -> {
            ...
            ProxyServer.getInstance().getPlayers().stream().filter(player -> player.hasPermission("polocloud.addon.notify")).forEach(player ->
                    player.sendMessage(MessageUtils.format(messagesConfiguration.serviceStopped().replace("%service%", event.cloudService().name()))));
        });
    }
}
