...
public class CloudServiceOnlineListener {
    ...
    public CloudServiceOnlineListener(ProxyServer server, MessagesConfiguration messagesConfiguration, EventNode node) {
        node.addListener(CloudServiceOnlineEvent.class, event -> {
            ...
            server.getAllPlayers().stream().filter(player -> player.hasPermission("polocloud.addon.notify")).forEach(player ->
                    player.sendMessage(MiniMessage.miniMessage().deserialize(messagesConfiguration.serviceStarted().replace("%service%", event.cloudService().name()))));
        });
    }
}
