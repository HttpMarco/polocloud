...
public class CloudServiceStartListener {
    ...
    public CloudServiceStartListener(MessagesConfiguration messagesConfiguration, EventNode node) {
        node.addListener(CloudServiceStartEvent.class, event -> {
            ...
            ProxyServer.getInstance().getPlayers().stream().filter(player -> player.hasPermission("polocloud.addon.notify")).forEach(player ->
                    player.sendMessage(MessageUtils.format(messagesConfiguration.serviceStarting().replace("%service%", event.cloudService().name()))));
        });
    }
}
