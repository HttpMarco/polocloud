...
class ConfigFactory(dataFolder: File) {
    ...
    var config: HubConfig = HubConfiguration().wrap()
            private set
    ...
    private fun loadOrCreateConfig() {
        ...
        runCatching {
            ...
            this.config = merged.wrap()
            saveConfig(merged)
        }.onFailure {
            println("Failed to load hub configuration: ${it.message}")
            this.config = HubConfiguration().wrap()
        }
    }


    fun saveConfig(configuration: HubConfiguration = (config as HubConfigAccessor).config) {
        runCatching {
