...
export async function POST(
  ...
  try {
    ...
    const response = await fetch(buildBackendUrl(decodedBackendIp, `/polocloud/api/v3/service/${serviceName}/command`), {
      method: 'POST',
      headers: {
    ...
    logError(error, { 
      component: 'ServiceCommand', 
      action: 'executeServiceCommand' 
    });
