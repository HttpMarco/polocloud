...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(buildBackendUrl(decodedBackendIp, '/polocloud/api/v3/groups/list'), {
      method: 'GET',
      headers: {
        'Cookie': `token=${token}`
      }
    ...
    if (response.ok) {
      ...
      return NextResponse.json({ error: errorData.message || 'Failed to fetch groups' }, { status: response.status });
    }
  } catch {
