...
export async function GET(
  ...
  try {
    ...
    const apiUrl = buildBackendUrl(decodedBackendIp, `/polocloud/api/v3/system/information/${type}?from=${from}&to=${to}`);
    ...
    if (response.ok) {
      ...
      return NextResponse.json({ 
        error: errorData.message || `Failed to fetch system ${type} data` 
