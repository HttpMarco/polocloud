...
import dev.httpmarco.polocloud.bridges.fabric.FabricBridge.logger
import dev.httpmarco.polocloud.bridges.fabric.mixin.ClientConnectionAccessor
import dev.httpmarco.polocloud.bridges.fabric.mixin.ServerLoginNetworkHandlerAccessor
...
class VelocityPacketHandler(val secret: String) {
    ...
    fun processVelocityLoginPacket(
        ...
        synchronizer.waitFor(server.submit {
            ...
            (handler as ServerLoginNetworkHandlerAccessor).profile = profile
        })
    }
    ...
    private fun updateConnectionAddress(handler: ServerLoginNetworkHandler, buf: PacketByteBuf) {
        ...
        (connection as ClientConnectionAccessor).address = InetSocketAddress(address, port)
    }
}