...
const AddonCard = ({ addon }: { addon: Addon }) => {
    ...
    return (
        <div
            className={`group relative bg-gradient-to-br from-card/30 via-card/20 to-card/30 backdrop-blur-xl border border-border/50 rounded-3xl p-8 transition-all duration-500 hover:scale-105 hover:-translate-y-2 shadow-xl hover:shadow-2xl`}
            onMouseEnter={() => setIsHovered(true)}
            onMouseLeave={() => setIsHovered(false)}
        >
            <div className={`absolute inset-0 bg-gradient-to-br ${addon.gradient} opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-3xl`} />
            ...
            <div className="relative z-10">
                <div className={`absolute top-6 right-6 px-4 py-2 rounded-full text-sm font-semibold border backdrop-blur-sm ${getCategoryColor(addon.category)}`}>
                    {getCategoryLabel(addon.category)}
                </div>

                <div className="mb-6">
                    <div className="flex items-center gap-4 mb-4">
                        <div className={`w-16 h-16 bg-gradient-to-br ${addon.gradient} rounded-2xl flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300`}>
                            <addon.icon className={`w-8 h-8 ${addon.color}`} />
                        </div>
                        <div>
                            <h3 className="text-2xl font-black text-foreground dark:text-white group-hover:text-primary transition-colors duration-300">
                                {addon.name}
                            </h3>
                ...
                <span className="text-sm text-muted-foreground dark:text-white/60">
                  v{addon.version}
                    ...
                    <p className="text-muted-foreground dark:text-white/70 leading-relaxed text-lg">
                        {addon.description}
                    </p>
                </div>

                <div className="mb-8">
                    <h4 className="text-lg font-bold text-foreground dark:text-white mb-4 flex items-center gap-2">
                        <Zap className="w-5 h-5 text-primary" />
                        Key Features
                    </h4>
                    <div className="grid grid-cols-1 gap-3">
                        {addon.features.map((feature, index) => (
                            <div key={index} className="flex items-center gap-3 text-muted-foreground dark:text-white/70 group/feature">
                                <div className="w-2 h-2 bg-primary rounded-full group-hover/feature:scale-150 transition-transform duration-300" />
                ...
                <div className="flex gap-4">
                    <button
                        ...
                        className="flex-1 inline-flex items-center justify-center gap-3 px-6 py-3 bg-gradient-to-r from-primary to-primary/90 hover:from-primary/90 hover:to-primary text-primary-foreground rounded-xl text-sm font-semibold transition-all duration-300 hover:scale-105 shadow-lg hover:shadow-xl group/btn"
                    >
                    ...
                    <button
                        ...
                        className="inline-flex items-center justify-center gap-3 px-6 py-3 bg-card/50 hover:bg-card border border-border/50 rounded-xl text-sm font-semibold transition-all duration-300 hover:scale-105 group/btn"
                    >
            ...
            <div className={`absolute inset-0 rounded-3xl border-2 border-primary/20 opacity-0 transition-opacity duration-500 pointer-events-none ${isHovered ? 'opacity-100' : ''}`} />
        </div>
    );
};
