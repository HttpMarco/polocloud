...
<Tabs items={['Kotlin', 'Java']}>
    <Tab value="Kotlin">
        <DynamicCodeBlock lang="kotlin" code='val eventProvider = PoloCloud.eventProvider()' />
    </Tab>
    <Tab value="Java">
        <DynamicCodeBlock lang="java" code='var eventProvider = PoloCloud.eventProvider();' />
    </Tab>
</Tabs>
...
eventProvider.subscribe(ServiceChangeStateEvent::class.java) { event ->
    ...
    when (service.state) {
        ServiceState.RUNNING -> {
            println("Service \${service.name()} is now running")
...
eventProvider.subscribe(ServiceChangeStateEvent.class, event -> {
    ...
    switch (service.getState()) {
        case RUNNING:
            System.out.println("Service " + service.name() + " is now running");
            // Start monitoring, send notifications, etc.
            break;
        case STOPPED:
...
The Event Provider gives you powerful tools to build event-driven applications and real-time communication across your entire cloud infrastructure! ⚡🚀
